...
export class UsersService {
  ...
  async login (loginUserDto: LoginUserDto): Promise<unknown> {
    ...
    /** On a décidé de ne pas implémenter un vrai système de connexion car un peu trop long à faire.**/
  ...
  async create (createUserDto: CreateUserDto): Promise<unknown> {
    ...
    if (await this.isUserExist(createdUser.pseudo)) {
      ...
      const user = await this.findOneConnected(createdUser.pseudo)
  ...
  async update (updateUserDto: UpdateUserDto): Promise<User> {
    if (
      !(await this.checkUserConnectedIsAdminOrAuthorized(
        updateUserDto.lastPseudo,
        updateUserDto.pseudoUserConnected
      ))
    ) {
      throw new HttpException(
        util.format(
    ...
    if (
      myUser &&
      (updateUserDto.lastPseudo === updateUserDto.newPseudo ||
        !(await this.isUserExist(updateUserDto.newPseudo)))
    ) {
      if (myUser.profileImageLink !== updateUserDto.profileImageLink) {
        ...
        const dateString =
          ...
          date.getSeconds()
        const newImageLink =
          updateUserDto.newPseudo + '_' + dateString.replace(/[ :]/g, '_')
  ...
  async checkUserConnectedIsAdminOrAuthorized (
    pseudo: string,
    pseudoUserConnected: string
  ): Promise<boolean> {
    if (pseudoUserConnected !== pseudo) {
      const userConnected = await this.userModel.findOne(
        { pseudo: pseudoUserConnected },
        { isAdmin: 1 }
      )
  ...
  async remove (deleteUserDto: DeleteUserDto) {
    if (
      !(await this.checkUserConnectedIsAdminOrAuthorized(
        ...
        deleteUserDto.pseudoUserConnected
      ))
    ) {
      throw new HttpException(
        util.format(
